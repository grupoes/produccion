...
$(function () {
  ...
  function setFormFromProveedor(p) {
    $("#form-proveedor").find('[name="nombre"]').val(p.nombre || "");
  }
  ...
  async function openEditModal(id) {
    try {
      ...
      editingId = id;
      $modalTitle.text(`Editar Proveedor #${id}`);
      $modalBody.html(formHtml());
      setFormFromProveedor(json.data);
      $btnGuardar.text("Actualizar").prop("disabled", false);
      bsModal.show();
    } catch (err) {
      showToast(err.message || "No se pudo cargar el proveedor.", "error");
    }
  }
  ...
  async function submitForm() {
    ...
    if (!data.nombre) {
      ...
      $("#form-proveedor [name='nombre']").focus();
      return;
    }
    ...
    try {
      ...
      bsModal.hide();
      $("#table-proveedores").DataTable().ajax.reload(null, false);
    } catch (err) {
  ...
  async function deleteProveedor(id, nombre) {
    ...
    try {
      ...
      showToast(json.message || "Proveedor desactivado.");
      $("#table-proveedores").DataTable().ajax.reload(null, false);
    } catch (err) {
  ...
  const table = $("#table-proveedores").DataTable({
    ...
    columns: [
      ...
      { data: "nombre", defaultContent: '<span class="text-muted">—</span>' },
      {
        ...
        className: "text-center",
        render: (_d, _t, row) => {
          const nombre = row.nombre || `proveedor #${row.id}`;
          return `
  ...
  $("#table-proveedores").on("click", ".btn-edit", function () {
    openEditModal(Number($(this).data("id")));
  });
  $("#table-proveedores").on("click", ".btn-delete", function () {
    const id = Number($(this).data("id"));
    const nombre = $(this).data("nombre");
    deleteProveedor(id, nombre);
  });

  // Limpia el body al cerrar

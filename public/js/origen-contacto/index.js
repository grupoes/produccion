...
$(function () {
  ...
  function formHtml() {
    return `
      <form id="form-origen" novalidate>
        <div class="row g-3">
          <div class="col-md-6">
  ...
  function setFormFromOrigen(o) {
    const $form = $("#form-origen");
    $form.find('[name="nombre"]').val(o.nombre || "");
    $form.find('[name="descripcion"]').val(o.descripcion || "");
  }
  ...
  async function openEditModal(id) {
    try {
      ...
      editingId = id;
      $modalTitle.text(`Editar Origen #${id}`);
      $modalBody.html(formHtml());
      setFormFromOrigen(json.data);
      $btnGuardar.text("Actualizar").prop("disabled", false);
      bsModal.show();
    } catch (err) {
      showToast(err.message || "No se pudo cargar el origen.", "error");
    }
  }
  ...
  async function submitForm() {
    ...
    try {
      ...
      bsModal.hide();
      $("#table-origenes").DataTable().ajax.reload(null, false);
    } catch (err) {
  ...
  async function deleteOrigen(id, nombre) {
    const ok = await confirmar({
      titulo: "¿Desactivar origen?",
    ...
    try {
      ...
      showToast(json.message || "Origen desactivado.");
      $("#table-origenes").DataTable().ajax.reload(null, false);
    } catch (err) {
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (_d, _t, row) {
          const nombre = row.nombre || `origen #${row.id}`;
          return `
  ...
  $("#btn-nuevo-origen").on("click", openCreateModal);
  $btnGuardar.on("click", submitForm);

  $("#table-origenes").on("click", ".btn-edit", function () {
    openEditModal(Number($(this).data("id")));
  });
  $("#table-origenes").on("click", ".btn-delete", function () {
    const id = Number($(this).data("id"));
    const nombre = $(this).data("nombre");
    deleteOrigen(id, nombre);
  });
